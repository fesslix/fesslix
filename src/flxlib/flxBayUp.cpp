...
void flxBayUp_adaptive_ctrl_dcs::write_adaptive_info(std::ostream& sout)
{
  ...
  sout << format("  sdR=%6.2e  sdW=%6.2e  ") % rv_Phi(cur_sdR_ut) % rv_Phi(cur_sdW_ut);
  if (cur_pSD>ZERO) {
    sout << format("sdS=%4.2f  ") % rv_Phi(cur_sdWS_ut);
    sout << format("pSD=%4.2f  ") % (cur_pSD);
  }
  if (omega_N>0) {
    sout << format("cosw=%4.2f  ") % (omega_sum/omega_N);
  }
}
...
{
  sout << format("  h=%4.2f   ") % rv_Phi(cur_sd_ut);
}
...
void FlxBayUp_Update_List::MHRS_uBUS(tdouble& p_mod)
{
  ...
  // compute the weights (w.r.t. maxL)
    ...
    // evaluate the weight vector
      ...
      w_i += (max_L - c_next);
      p_mod += w_i;
      ext_out << format("qw=%4.2f ") % exp(w_i);
  // perform a re-sampling to alter the sampling distribution
    ...
    // perform the resampling
      ...
      acr /= (N_in_list-1);
      ext_out << format("qacr=%4.2f qCoV=%4.2f ") % acr % w_cov;
  // correct s_thr
    s_thr += max_L - c_next;
    ext_out << format("gt2=%9.2e ") % s_thr;
}
...
void FlxBayUP_csm_cwmh_MCMC::write_adaptive_info(std::ostream& sout, const bool is_adaptive)
{
  ...
  sout << format("  h=%4.2f   ") % kernel->get_h();
}
...
//       delta[i] = sqrt((tdouble(cls.Nsamples-cls.Nfailures+inv_eff)*inv_eff)/((cls.Nsamples+3*inv_eff)*(cls.Nfailures+inv_eff)));
  ...
  // bias
    scout3 << "        upper bound for bias: " << format("%9.2e") % (p2.cast2double()/2)
      ...
      for ( tuint i=0; i<pcM->get_nrows(); ++i ) {
        for ( tuint j=0; j<pcM->get_ncols(); ++j ) {
          ...
          // Calculate the credible interval
            ...
            crp[crp_id++] = bay_low;
            crp[crp_id++] = bay_up;
            scout3 << "[" << format("%10.3e") % bay_low << ";" << format("%10.3e") % bay_up << "]" << std::endl;
        }
      }
      ...
      for ( tuint i=0; i<pcM->get_nrows(); ++i ) {
        for ( tuint j=0; j<pcM->get_ncols(); ++j ) {
          ...
          // Calculate the cerdible interval
            ...
            crp[crp_id++] = bay_ub;
            scout3 << "       " << format("%10.3e") % bay_ub << std::endl;;
        }
      }
  } catch (FlxException& e) {
...
void FlxBayUp_Update::update(FlxBayUp_Update_List* listV, FlxBayUP_csm_base* csmV, const bool use_cStart, const Flx_SuS_Control& susControlV)
{
  ...
  scout << "  Number of random variables:   " << format("%-10.0f") % list->get_Nrv();
    if (list->get_Nrv()!=list->get_NOX()) {
      scout << " (standard normal space)"<< std::endl;
      scout << "                                " << format("%-10.0f") % list->get_NOX() << " (original space)";
    }
    scout << std::endl;
  ...
  if (csm) {
    ...
    scout << "  Threshold level:              " << format("%4.2f") % (ONE/tdouble(list->get_Ncl())) << std::endl;
    }
  ...
  switch (list->meth_id) {
    ...
    // --------------------------------------------------------------------------------------------------------------------------
      ...
      // do MCI in a first step
        ...
        switch (list->meth_id) {
          ...
          case (FlxBayUp_Update_List::BUST):
          {
            ...
            if (mhml>=log(GlobalVar.TOL()) && mhml<=100.) {
              scout << format("maxL=%9.2e  ") % exp(mhml) << "           ";
            } else {
              scout << format("mlnL=%9.2e  ") % mhml << "           ";
            }
            break;
          }
          case (FlxBayUp_Update_List::ABCSUBSIM):
        ...
        scout << format("velo=%4.2f  ") % list->get_velo();
            ...
            switch (list->meth_id) {
              ...
              case (FlxBayUp_Update_List::RASUBSIM):
              {
                if (gt_is_zero) {
                  ...
                  scout << format("gt=%9.2e") % list->get_thr();
                }
                break;
              }
              case (FlxBayUp_Update_List::BUST):
            ...
            // compute the effective number of samples
              if (susControl.comp_gamma && Nc_now!=curLevelStat->Nsamples) {        // compute gamma
                ...
                if (susControl.verbose && N_runs>1) {
                  scout_ext << format("Ge=%2.0f") % (curLevelStat->eff_Gelman*100) << "% ";
                  scout_ext << format("eff=%2.0f") % (curLevelStat->eff*100) << "% ";
                  scout_ext << format("lag1c=%2.0f") % (curLevelStat->lag1_corr*100) << "% ";
                  if (N_runs>2 && susControl.consider_seed_corr) {
                    scout_ext << format("sc2eff=%2.0f") % (curLevelStat->gamma_from_seed/curLevelStat->gamma*100) << "% ";
                    if (curLevelStat->corr_pi_prev>0.01) {
                      scout_ext << format("piCorr=%2.0f") % (curLevelStat->corr_pi_prev*100) << "% ";
                      #if FLX_DEBUG
                        scout_ext << format("piCnF=%1.1f") % (curLevelStat->corr_pi_prev_negFrac) << " ";     
                      #endif
                    }
                  }
          ...
          // output acceptance rate
            {
              ...
              scout << format("acr=%4.2f  ") % acr_val;
              if (susControl.verbose && acr_val>GlobalVar.TOL()) {
                if (list->get_Nrv()>1 && csm->get_ac1d()<0.995) {
                  scout_ext << format("ac1d=%4.2f ") % csm->get_ac1d();
                  scout_ext_nempty = true;
                }
                const tdouble acr1 = ONE-tdouble(nac1)/tdouble(tc2);
                if (acr1<0.995) {
                  scout_ext << format("acr1=%4.2f ") % (acr1);
                  scout_ext << format("acr2=%4.2f ") % (ONE-tdouble((nac-nac1))/tdouble(tc2-nac1));
                  scout_ext_nempty = true;
                }
              }
            }
          // output velocity
            scout << format("velo=%4.2f  ") % list->get_velo(Nc_now);
          ++N_runs;
          ...
          // output samples of the last step
            ...
            }
            {
              ...
              if (exp(l_max_level)>=GlobalVar.TOL() && l_max_level<=100.) {
                scout << format("maxL=%9.2e  ") % exp(l_max_level);
              } else {
                scout << format("mlnL=%9.2e  ") % l_max_level << " ";
              }
            }
